<template>
  <CommonLayout :page-name="currentPageName">
    <template #main>
...
import { songApi, likeApi } from '@/api';
import type { ModelsSongDetailDTO, SongsGetSortByEnum } from '@/api-client';
...
const currentPageName = ref<string>('公共歌单');
...
const sortBy = ref<SongsGetSortByEnum>('latest');
...
const loadData = async () => {
  ...
  try {
    const [songsResponse, likedSongsResponse] = await Promise.all([
      songApi.songsGet(page.value, pageSize.value, searchQuery.value, sortBy.value),
      currentUserId.value ? likeApi.meLikedSongsGet(1, 1000) : Promise.resolve(null),
    ]);
